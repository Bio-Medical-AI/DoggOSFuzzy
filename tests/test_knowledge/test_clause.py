...
class TestClause:
    ...
    def test_get_value_type1fuzzy_set(self, x):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
    ...
    def test_get_value_collection_type1fuzzy_set(self, x, y, z):
    ...
    @pytest.mark.parametrize('x', np.arange(0, 10, 0.5))
    def test_get_value_interval_type2fuzzy_set(self, x):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        ...
        values = fuzzy_set(domain())
        index = np.round((x - domain.min) / domain.precision).astype(int)
        assert np.array_equal(np.take(values, index, axis=-1), clause.get_value(x))

    @pytest.mark.parametrize('x, y, z', zip(np.arange(0, 10, 1), np.arange(2, 12, 1), np.arange(3, 13, 1)))
    def test_get_value_collection_interval_type2fuzzy_set(self, x, y, z):
        collection = np.array([x, y, z])
        domain = Domain(0, 15, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        upper_mf = lambda x: 1 / (1 + np.exp(-x))
        ...
        values = fuzzy_set(domain())
        index = np.round((collection - domain.min) / domain.precision).astype(int)
        assert np.array_equal(np.take(values, index, axis=-1), clause.get_value(collection))

    @pytest.mark.parametrize('x', np.arange(-5, -1, 1))
    def test_exception_value_error_get_value_interval_type2fuzzy_set(self, x):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        fuzzy_set = Type1FuzzySet(lambda x: 0.5 * x)
        clause = Clause(ling_var, 'High', fuzzy_set)
    ...
    def test_exception_typerror_setter_type1fuzzy_set(self):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        fuzzy_set = Type1FuzzySet(lambda x: 0.5 * x)
        clause = Clause(ling_var, 'High', fuzzy_set)
    ...
    def test_setter_exception_typeerror_values_type1fuzzy_set(self):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        fuzzy_set = Type1FuzzySet(lambda x: 0.5 * x)
        clause = Clause(ling_var, 'High', fuzzy_set)
    ...
    def test_setter_exception_typeerror_values_type2fuzzy_set(self):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        upper_mf = lambda x: 1 / (1 + np.exp(-x))
        lower_mf = lambda x: 1
        ...
        clause = Clause(ling_var, 'High', fuzzy_set)
        func = lambda x: 0.7 * x
        new_domain = Domain(0, 10, 0.01)
        new_values = func(new_domain())
        with pytest.raises(ValueError) as e:
            clause.values = new_values
            assert 'Values length mismatches domain of linguistic variable' in str(e.value)

    def test_getter_values_type1fuzzy_set(self):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        fuzzy_set = Type1FuzzySet(lambda x: 0.5 * x)
        clause = Clause(ling_var, 'High', fuzzy_set)
    ...
    def test_getter_values_interval_type2fuzzy_set(self):
        domain = Domain(0, 10, 0.01)
        ling_var = LinguisticVariable('Temperature', domain)
        upper_mf = lambda x: 1 / (1 + np.exp(-x))
        lower_mf = lambda x: 1
