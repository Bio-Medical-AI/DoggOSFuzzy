...
def main():
    ...
    for train_index, test_index in skf.split(train, train_y):
        ...
        ling_vars, fuzzy_sets, clauses = create_set_of_variables(['Pregnancies','Glucose','BloodPressure',
                                                                  'SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age','Outcome'],
                                                                                                      fuzzy_set_type='it2')
        # generate fuzzy decision table
        gen = FuzzyDecisionTableGenerator(train_data, train_y, fuzzy_sets, clauses, target_label='Outcome')
        fuzzified_dataset = gen.fuzzify()

        induction = InformationSystem(train_data, train_y, ['Pregnancies','Glucose','BloodPressure',
                                                                  'SkinThickness','Insulin','BMI','DiabetesPedigreeFunction','Age'])
        antecedends, _ = induction.induce_rules(fuzzy_sets, clauses)
