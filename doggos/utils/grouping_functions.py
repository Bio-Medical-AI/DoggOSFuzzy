...
from doggos.utils.membership_functions import generate_equal_gausses, generate_progressive_gausses
...
def create_triangular_t1(n_mfs, domain: Domain = Domain(0, 1.001, 0.001)):
    fuzzy_sets = generate_even_triangulars(n_mfs, domain.min, domain.max)
    return fuzzy_sets


def create_trapezoidal(n_mfs, domain: Domain = Domain(0, 1.001, 0.001)):
    fuzzy_sets = generate_even_trapezoidal(n_mfs, domain.min, domain.max)
    return fuzzy_sets
...
def create_set_of_variables(ling_var_names: Sequence[str],
    ...
    if fuzzy_set_type == 't1':
        ...
        if mf_type == 'gaussian':
            fuzzy_sets = create_gausses_t2(n_mfs=n_mfs)
        elif mf_type == 'triangular:':
            fuzzy_sets = create_triangular_t2(n_mfs=n_mfs)
        elif mf_type == 'trapezoidal':
            fuzzy_sets = create_trapezoidal_t2(n_mfs=n_mfs)
        else:
            raise Exception(f"mf_type cannot be of type {mf_type}")
